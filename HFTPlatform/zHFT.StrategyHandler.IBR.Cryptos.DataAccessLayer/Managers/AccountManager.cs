...
using zHFT.StrategyHandler.IBR.Cryptos.DataAccess;
...
using zHFT.StrategyHandler.InstructionBasedRouting.Cryptos.DataAccessLayer;


namespace zHFT.StrategyHandler.IBR.Cryptos.DataAccessLayer.Managers
{
    public class AccountManager : MappingEnabledAbstract, IAccountManagerAccessLayer
    {
        ...
        public Account GetByAccountNumber(int accoutNumber)
        {
            accounts accountDB = ctx.accounts.Where(x => x.account_number == accoutNumber).FirstOrDefault();
