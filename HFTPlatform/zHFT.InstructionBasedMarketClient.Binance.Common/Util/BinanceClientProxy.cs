    ...
    public class BinanceClientProxy : BinanceClient
    {
        ...
        { 
           if (string.IsNullOrWhiteSpace(symbol))
            ...
            string args = string.Format("symbol={0}&limit={1}",symbol, 5);

            var result = await _apiClient.CallAsync<dynamic>(ApiMethod.GET, EndPoints.OrderBook, false, args);

            var parsedResult = GetParsedOrderBook(result);
